  ...
  const CSSMotion = React.forwardRef<any, CSSMotionProps>((props, ref) => {
    ...
    function getDomElement() {
      try {
        ...
        return nodeRef.current instanceof HTMLElement
          ? nodeRef.current
          : findDOMNode<HTMLElement>(wrapperNodeRef.current as any);
      } catch (e) {
    ...
    if (!children) {
      ...
      motionChildren = children(
        {
          ...mergedProps,
          className: classNames(getTransitionName(motionName as any, status), {
            [getTransitionName(motionName as any, `${status}-${statusSuffix}`)]:
              statusSuffix,
    ...
    return <DomWrapper ref={wrapperNodeRef as any}>{motionChildren}</DomWrapper>;
  });
